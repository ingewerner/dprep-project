...
# See whether sentimentscores differ with the year they are tweeted in
one.way <- aov(sentimentscores ~ year_tweeted, data = BLM_merged)
summary(one.way)

# See whether likes and retweets have an impact on followers of the user
two.way <- aov(user_amount_followers ~ like*retweet, data = BLM_merged)
summary(two.way)

#how to make this one? Effect of sentiment and the year on the amount of tweets
two_ANOVA <- aov(amount of tweets ~ sentiment*year_tweeted, data = BLM_merged)
summary(two.way)
...
#We have to filter on user_id for this, user_id has a lot of e+, I made new variables within the data at the preclean stage
sum(duplicated(blmdata$user_id_full)) #to see if there are duplicates? 
unique(blmdata$user_id_full, incomparables = FALSE)
